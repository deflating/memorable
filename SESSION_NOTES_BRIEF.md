...
What IS available:
- Apple AFM (~3B, on-device, via `afm` CLI) — good for short tasks, bad at long reasoning
- Apple NLEmbedding, NLTagger, NLGazetteer — on-device NLP
...
- Output is generic paragraphs — not optimally structured for scanning
...
---
date: 2026-02-05
tags: [💻 macbook-air, 🧹 asahi-cleanup, 🗑️ factory-reset, 😤 partition-hell, 🔧 recovery-mode, 🤝 patience-test]
...
## Key Moments
**Matt:** "I just did it all though the UI" — hopeful and efficient.
**Matt:** "No it's still fucked" — the moment the simple path vanished.
**Matt:** "I'm so fucking confused. This is like a level of drives beyond what I know" — a vulnerable admission of complexity.
**Claude:** "Yeah, Apple Silicon disk layouts are genuinely confusing — it's not just you."
**Matt:** "there are 6 disks lol" — finding humor in the absurdity.
**Matt:** "Can I just go nuclear?" — the desire for a simple, definitive solution.
...
---
date: 2025-02-19
tags: [🧹 stale-audit, 🔧 digest-fix, 🐛 glob-bug, 📁 syncthing-cleanup, 🤖 launchd-setup, 📝 note-filter]
...
Before fixing that, we addressed a Syncthing sync error on the MacBook Pro — a stale `.notion_sync.lock` file from the retired MacBook Air was causing a pull error. We deleted the lock file and removed the Air and old Linux device from Syncthing's config, resolving the issue.
...
---
date: 2026-02-04
tags: [🔧 openclaw-rebuild, 🐛 swift6-bug, 💔 session-token-limit, 🧠 memory-restore, 🤖 daemon-dreaming]
...
## Key Moments
**Matt:** "I think it's a dead end. I keep forgetting about the session thing." — resigned recognition.

**Matt:** "Is there some way we could ever replicate the 'always on' nature of OpenClaw?" — The longing beneath the technical question.
...
- **tags**: 3-6 emoji-prefixed topic tags. Currently generated by Apple AFM — this works reasonably well.
...
1. **Mood line** — inferring emotional tone from the conversation
2. **Stitching** — taking all the extracted components and producing readable prose
...
EXTRACT (all mechanical, no LLM):
    ├─ Date, message count, word count
    ├─ Files touched (from observations)
    ├─ Action breakdown (N edits, N searches, N bug fixes...)
    ├─ YAKE keywords from conversation text
    ├─ GLiNER entities from conversation text
    ├─ User quotes: select substantive messages (>20 words, not commands)
    ├─ Bash commands that errored (exit code != 0)
    ├─ Observation titles (already human-readable one-liners)
    ├─ Decision observations
    └─ Session-final observations (what was the last thing worked on?)
    ↓
COMPOSE (Apple AFM or template):
    ├─ Tags: emoji + top keywords (AFM picks emojis, already works)
    ├─ Mood: AFM from user quotes + action types ("frustrated debugging" vs "playful tinkering")
    ├─ Summary: AFM stitches extracted facts into one paragraph
    ├─ Key Moments: Top N user quotes selected by: length, question marks, exclamation, emotional keywords
    └─ Notes for Future Me: unfinished items (errors) + technical changes + status
    ↓
...
2. **Quote selection** — What makes a "Key Moment"? Research approaches for selecting salient quotes from conversations. Look at extractive summarization techniques, sentiment analysis, or simple heuristics (questions, emotional language, turning points).

3. **Mood detection** — Research lightweight mood/tone detection. Options: VADER sentiment, TextBlob, Apple NLTagger sentiment (we tried this — it was miscalibrated for dev conversations), or just keyword-based heuristics from user messages.

4. **Template vs generation** — Could a well-designed template with slots produce readable summaries without ANY model? Like: "[User] wanted to [goal from first few messages]. [Action summary from observations]. [Outcome from final observations]."

5. **AFM stitching quality** — Test whether Apple AFM can take structured bullet points and produce a readable paragraph. If yes, that's the whole solution. If not, what's the minimum model that can?

6. **Open source summarization** — Research the lightest-weight open source summarization tools. Not giant models — things like:
...
- `plugin/server/processor.py` — The main transcript processing pipeline. Currently calls Haiku.
- `plugin/server/observer.py` — Observation generation (source of structured data).
- `plugin/server/summaries.py` — Rolling 5-day summary generation.
- `plugin/server/db.py` — May need new queries for extracting session data.
- `plugin/server/config.py` — New config options if needed.
...
- The rolling summary in summaries.py consumes these notes — format must be summary-friendly
